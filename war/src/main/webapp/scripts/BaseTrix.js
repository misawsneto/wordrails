...
function FileDto(id, directory, hash, mime, name, size, type, url) {
	return {
		id: id,
		directory: directory,
		hash: hash,
...
function PersonDto(id, bio, bookmarkPosts, coverHash, coverLargeHash, coverMediumHash, coverOriginalHash, email, imageHash, imageLargeHash, imageMediumHash, imageOriginalHash, imageSmallHash, lastLogin, name, twitterHandle, username) {
	return {
		id: id,
		bio: bio,
		...
		coverOriginalHash: coverOriginalHash,
		email: email,
		imageHash: imageHash,
		imageLargeHash: imageLargeHash,
		imageMediumHash: imageMediumHash,
		imageOriginalHash: imageOriginalHash,
...
function PostDto(id, author, body, bookmarksCount, commentsCount, date, externalFeaturedImgUrl, externalVideoUrl, featuredAudioHash, featuredImage, featuredVideoHash, imageCaptionText, imageCreditsText, imageHash, imageId, imageLandscape, imageLargeHash, imageMediumHash, imageSmallHash, imageTitleText, lastModificationDate, lat, lng, notify, originalPostId, originalSlug, readTime, readsCount, recommendsCount, scheduledDate, slug, state, station, stationId, subheading, tags, title, topper) {
	return {
		id: id,
		author: author,
		body: body,
...
.provider('trix', function trixProvider($httpProvider) {
  ...
  var Trix = function($http){
    ...
    this.getStationPages = function(stationId) {
    ...
    this.forceDeleteStation = function(stationId) {
  	...
  	/*---------------------------------------------------------------------------*/
  	    ...
  	    if (this.findByAuthorUsernameAndState) {
  	    	window.console && console.log("findByAuthorUsernameAndState");
  	    }
  	    this.findByAuthorUsernameAndState = function(username, state, page, size, sort, projection) {
  	        var config = {};
  	        config.params = {
  	            username: username,
  	            state: state,
  	        ...
  	        config.params["projection"] = projection;
  	        return $http.get(_config.url + "/api/posts/search/findByAuthorUsernameAndState",  config)
  	    };
