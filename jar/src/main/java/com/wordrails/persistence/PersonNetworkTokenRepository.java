...
//	 where s.visibility = "UNRESTRICTED"))
	...
	@RestResource(exported=false)
	@Query("SELECT token FROM PersonNetworkToken token where token.person in (select psr.person from StationRole psr where psr.station.id = 2)")
	public List<PersonNetworkToken> findByStationId(@Param("stationId") Integer stationId);
