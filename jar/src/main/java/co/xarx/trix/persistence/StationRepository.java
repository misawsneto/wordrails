...
public interface StationRepository extends JpaRepository<Station, Integer>, QueryDslPredicateExecutor<Station> {
    ...
    @Query("select station from Station station join station.personsStationRoles personRoles join personRoles.person person where :networkId = station.network.id and (person.id = :personId or station.visibility = 'UNRESTRICTED') GROUP BY station")
