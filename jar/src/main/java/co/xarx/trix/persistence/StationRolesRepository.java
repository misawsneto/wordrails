...
public interface StationRolesRepository extends DatabaseRepository<StationRole, Integer> {
	...
	@RestResource(exported = false)
	@Query("select count(*) FROM StationRole str WHERE str.station.id in (:stationIds) order by str.person.id desc")
	...
	@RestResource(exported = false)
	@Query("select count(*) FROM StationRole str WHERE str.station.id in (:stationIds) AND (str.person.name = :nameOrUsernameOrEmail OR str.person.username = :nameOrUsernameOrEmail OR str.person.email = :nameOrUsernameOrEmail) order by str.person.id desc")
